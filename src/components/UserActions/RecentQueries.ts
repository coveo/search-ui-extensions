...
export class RecentQueries extends Component {
     ...
     */
    static readonly ID = 'RecentQueries';

    /**
     * Default initialization options of the **RecentQueries** class.
     */
    static readonly options: IRecentQueriesOptions = {
    ...
    private sortedRecentQueries: string[];

    /**
     * Create an instance of **RecentQueries**. Initialize is needed the **UserProfileModel** and fetch user actions related to the **UserId**.
     *
    ...
    constructor(public element: HTMLElement, public options: IRecentQueriesOptions, public bindings: IComponentBindings) {
        ...
        this.userProfileModel.getActions(this.options.userId).then(actions => {
            this.sortedRecentQueries = [...actions]
    ...
    private render() {
        new ExpandableList<string>(this.element, this.sortedRecentQueries, {
            maximumItemsShown: this.sortedRecentQueries.length,
            ...
            messageWhenEmpty: l(`${RecentQueries.ID}_no_queries`),
            showMoreMessage: l(`${RecentQueries.ID}_more`),
            showLessMessage: l(`${RecentQueries.ID}_less`)
        });
    }
